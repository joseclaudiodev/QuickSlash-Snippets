...
document.addEventListener('DOMContentLoaded', async () => {
	...
	if (appData.categories && appData.categories.length > 0) selectCategory(appData.categories[0].id);
...
function renderActiveCategory() {
	...
	category.urlPatterns.forEach(url => {
		...
		updateUrlsTruncatedClass();
	});

	renderSnippets();
}

function updateUrlsTruncatedClass() {
	const el = document.getElementById('current-category-urls');
	if (!el) return;
	if (el.scrollHeight > el.clientHeight) el.classList.add('is-truncated');
